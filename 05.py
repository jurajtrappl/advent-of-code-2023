...
with open("05.in", "r") as f:
    ...
    raw_seeds, raw_maps = sections[0].split()[1:], [m.split(os.linesep)[1:] for m in sections[1:]]
    seeds, maps = [int(seed) for seed in raw_seeds], [[[int(x) for x in row.split()] for row in m] for m in raw_maps]
    
p1_values = seeds
p2_values = [(seed_start, seed_start + range_length) for seed_start, range_length in zip(seeds[::2], seeds[1::2])]
for m in maps:
    tree = IntervalTree()
    for destination_start, source_start, range_length in m:
        tree.addi(source_start, source_start + range_length, (destination_start, destination_start + range_length))
    ...
    for start, end in p2_values:
        ...
        for interval in sorted(intervals):
            ...
            p2_new_values.append((interval.data[0] + overlapping_start - interval.begin, interval.data[0] - interval.begin + overlapping_end))
